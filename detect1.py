...
# #결과 옮길 csv 파일 불러오기
# df1 = pd.read_csv('C:/Users/202210/Desktop/Python Project/yolov5_sdent_detect_model/result1.csv')
...
def run(
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            # elif (len(det) == 1) :
            #     if (c == 0) :
            #         file_result = 'Under' #축소용접
            #         df1.at[now_num,'용접 불량 진단 개수'] += 1 #해당 날짜에 용접 불량 용접불량 기록
            #         df1.to_csv("C:/Users/202210/Desktop/Python Project/yolov5_sdent_detect_model/result1.csv",mode='w')
            #     elif (c == 1) :
            #         file_result = 'Over' #과용접
            #         df1.at[now_num,'용접 불량 진단 개수'] += 1 #해당 날짜에 용접 불량 용접불량 기록
            #         df1.to_csv("C:/Users/202210/Desktop/Python Project/yolov5_sdent_detect_model/result1.csv",mode='w')
            #     elif (c == 2) :
            #         file_result = 'Empty' #시료가 없음,빈상태
...
def parse_opt():
    ...
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'cam1_detect.pt', help='model path or triton URL') 
    #가중치: 축간 불량 분석시 'cam2_detect.pt'사용, 용접 불량 분석시 'cam1_detect.pt'사용
    parser.add_argument('--source', type=str, default=ROOT / 'Store/CAM1/', help='file/dir/URL/glob/screen/0(webcam)') 
    ...
    parser.add_argument('--name', default='test_dataset', help='save results to project/name') 
...
def main(opt):
    ...
    run(**vars(opt))
    # f = open('warehouse1.txt','w') #감지된 결과 텍스트로 저장
    # f.write(file_result)
    # f.close()
